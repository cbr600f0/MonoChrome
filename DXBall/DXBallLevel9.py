...
class DXBallLevel9 (SceneManager.Scene):
    ...
    def __init__(self):
        ...
        self.MainBG = pygame.image.load('DXBall\Images\Level9.png').convert_alpha()
        ...
        blockStartX = 100
        for i in range(12):
            spawnPos = Vector2(blockStartX + 100 * (i + 1) + (10 * i), 100)

            Block(spawnPos, self.blockSprites, self.ballcollideSprites, self.allSprites)

        blockStartX = 260
        for i in range(9):
            spawnPos2 = Vector2(blockStartX + 100 * (i + 1) + (10 * i), 200)
        ...
        blockStartX = 580
        for i in range(3):
            spawnPos4 = Vector2(blockStartX + 100 * (i + 1) + (10 * i), 400)

            Block2(spawnPos4, self.blockSprites, self.ballcollideSprites, self.allSprites)

        blockStartX = 690
        for i in range(1):
            spawnPos5 = Vector2(blockStartX + 100 * (i + 1) + (10 * i), 500)
