...
class DXBallLevel3 (SceneManager.Scene):

    def __init__(self):
        super(DXBallLevel3, self).__init__()
        ...
        self.MainBG = pygame.image.load('DXBall\Images\Level3.png').convert_alpha()
        ...
        blockStarty = 0
        for i in range(7):
            spawnPos3 = Vector2(50, blockStarty + 100 * (i + 1) + (10 * i))

            Block2(spawnPos3, self.blockSprites, self.ballcollideSprites, self.allSprites)

        blockStarty = 0
        for i in range(7):
            spawnPos4 = Vector2(1450, blockStarty + 100 * (i + 1) + (10 * i))
    ...
    def render(self, screen):
        ...
        CurrentLevel = self.DXBallFont.render("Level: 3", False, (2, 255, 149))
